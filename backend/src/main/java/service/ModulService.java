...
import model.Modul;
import model.dto.ModulDTO;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;
import repository.ModulRepository;
    ...
    public Modul toEntity(ModulDTO modulDTO){
        ...
        modul.setCreator(
                applicationUserService.toEntity(
                        applicationUserService.findById(modulDTO.getCreatorId())
                )
        );
        modul.setClasses(
                modulDTO.getClassroomIds()
                        .stream().map(classroomId ->
                                classroomService.toEntity(classroomService.findById(classroomId)))
                        .collect(Collectors.toSet())
        );
